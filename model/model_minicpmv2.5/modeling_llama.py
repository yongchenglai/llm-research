    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        if layer_idx is None:
            logger.warning_once(
                ...
                f"a `layer_idx` is not recommended and will "
...
class LlamaFlashAttention2(LlamaAttention):
    """
    Llama flash attention module. This module inherits from `LlamaAttention`
    as the weights of the module stays untouched.
    The only required change would be on the forward pass where it needs to
    correctly call the public API of flash attention and deal with
    ...
    def forward(
        ...
        if isinstance(past_key_value, StaticCache):
            raise ValueError(
                ...
                "make sure to use `sdpa` in the mean time, and open an issue "
    ...
    def _flash_attention_forward(
        ...
        if not self._flash_attn_uses_top_left_mask:
            ...
            # check once Flash Attention for RoCm is bumped to 2.1. For details,
        ...
        if attention_mask is not None:
            ...
            query_states, key_states, value_states, indices_q, \
                cu_seq_lens, max_seq_lens = self._upad_input(
                   query_states, key_states, value_states,
