...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, unsqueeze_dim=1):
    ...
    Args:
        ...
        position_ids (`torch.Tensor`):
            ...
            For example, this can be used to pass offsetted position ids
            when working with a KV-cache.
        unsqueeze_dim (`int`, *optional*, defaults to 1):
            The 'unsqueeze_dim' argument specifies the dimension along
            which to unsqueeze cos[position_ids] and
            sin[position_ids] so that they can be properly broadcasted to
            the dimensions of q and k. For example, note
...
class IdeficsAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.is_cross_attention:
            kv_input_dim = (
                self.hidden_size if not hasattr(config.vision_config, "embed_dim")
    ...
    def _shape(self, tensor: torch.Tensor, seq_len: int, bsz: int):
        return tensor.view(bsz, seq_len,
                           self.num_heads, self.head_dim).transpose(1, 2).contiguous()
    ...
    def forward(
        ...
        if not is_cross_attention:
            ...
            key_states = self.k_proj(key_value_states).view(
                bsz, kv_len, self.num_heads, self.head_dim).transpose(1, 2)
            value_states = (
                self.v_proj(key_value_states).view(
                    bsz, kv_len, self.num_heads, self.head_dim).transpose(1, 2)
            )
        ...
        is_causal = True if self.is_causal \
                            and attention_mask is None \
                            and q_len > 1 else False
        ...
        if output_attentions:
            logger.warning_once(
                "attn_weights are not extracted in scaled_dot_product_attention. "
                "The model returns None instead"
            )
    ...
    def forward(
        ...
        Args:
            ...
            use_cache (`bool`, *optional*):
                ...
                (see `past_key_values`).
            past_key_value (`Tuple(torch.FloatTensor)`, *optional*): cached past key
