...
def upload_video(gr_video):
    if gr_video is None:
        return None, gr.update(interactive=True,
                               placeholder='Please upload video/image first!'), \
...
def gradio_ask(user_message, chatbot):
    if len(user_message) == 0:
        return gr.update(interactive=True,
                         placeholder='Input should not be empty!'), chatbot
...
def gradio_answer(video_path, chatbot, num_beams, temperature):
    ...
    response = agent.answer(video_path=video_path,
                            prompt=chatbot[-1][0],
                            max_new_tokens=200,
                            num_beams=num_beams,
        ...
        with gr.Row():
            with gr.Column(scale=0.5, visible=True) as video_upload:
                with gr.Tab("Video", elem_id='video_tab'):
                    up_video = gr.Video(interactive=True,
                                        include_audio=True,
