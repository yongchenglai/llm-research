...
class AtomTRTApi:
    ...
    def ask(self, input_text, temperature=0.4, top_p=0.95,
            max_new_tokens=1024, repetition_penalty=1.2,
            system_prefix='', merge_lambda=None,
            max_input_length=4096, append_next_role=True):
        with torch.no_grad():
            prompt = ''
            print('max_input_length', max_input_length)
            if type(input_text) == list:
                for input_text_one in input_text[::-1]:
                    if len(prompt) + len("<s>" + input_text_one['role'] + ": " + input_text_one[
                        'content'].strip() + "\n</s>") < max_input_length:
                        prompt = "<s>" + input_text_one['role'] + ": " + input_text_one[
                            'content'].strip() + "\n</s>" + prompt
                if append_next_role:
            ...
            bot_message = self.tokenizer.batch_decode(
                generate_ids,
    ...
    def ask_streaming(
            ...
            append_next_role=True):
        with torch.no_grad():
            prompt = ''
            print('max_input_length', max_input_length)
            if type(input_text) == list:
                for input_text_one in input_text[::-1]:
                    if len(prompt) + len("<s>" + input_text_one['role'] + ": "
            ...
            for curr_outputs in throttle_generator(generate_ids, 2):
                ...
                answer_message = self.tokenizer.batch_decode(
                    output_ids,
                    skip_special_tokens=False,
                    clean_up_tokenization_spaces=False)[0]
